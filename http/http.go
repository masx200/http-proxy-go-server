...
// resolveTargetAddressForHttp 解析目标地址的域名为IP地址（用于http模块）
...
// resolveTargetAddressForHttpWithRoundRobin 从解析的IP数组中轮询选择一个地址（http模块使用）
...
// shuffleHttp 对切片进行随机排序（http模块使用）
func shuffleHttp[T any](slice []T) []T {
