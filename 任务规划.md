...
- 已扩展 SelectProxyURLWithCIDR 函数，在所有代理选择逻辑中优先选择 WebSocket 代理
...
- [x] 添加 overrideProxyURLCredentials 辅助函数，用于解析代理 URL 并用配置中的用户名密码覆盖 URL 中的对应信息
- [x] 支持仅覆盖用户名、仅覆盖密码或同时覆盖两者的场景
- [x] 在 SelectProxyURLWithCIDR 函数中集成 overrideProxyURLCredentials 函数
- [x] 支持 http_username 和 http_password 覆盖 HTTP_PROXY 和 HTTPS_PROXY 的 URL 中的用户名密码
...
- 现有架构已支持 SOCKS5 代理配置，UpStream 结构体包含 SOCKS5_PROXY、SOCKS5_USERNAME、SOCKS5_PASSWORD 字段
- 已实现完整的 socks5DialContext 函数，支持代理 URL 解析、客户端创建和连接处理
- SOCKS5 代理已集成到 transportConfigurations 中，通过自定义 DialContext 函数实现
...
- UpStream 结构体已包含完整的 SOCKS5 代理配置字段：SOCKS5_PROXY、SOCKS5_USERNAME、SOCKS5_PASSWORD
- JSON 解析逻辑自动支持新字段，无需额外修改
...
- 已支持 SOCKS5 代理相关命令行参数：-socks5-proxy、-socks5-username、-socks5-password
...
- transportConfigurations 已正确集成 SOCKS5 代理，通过自定义 DialContext 函数实现
...
4. **文档更新**: 更新 README.md，添加所有代理类型的详细使用说明 if proxyURL != nil
   ...
   IPv6 地址 upstreamAddress = "[" + upstreamAddress + "]:80" } else { //
   域名或 IPv4 地址 upstreamAddress = upstreamAddress + ":80" } host, port, err =
   ...
   // 在 goroutine 中处理 WebSocket 数据转发 go func() { defer clientConn.Close()
   defer serverConn.Close() // 使用 ForwardData 方法处理 WebSocket 连接 err :=
   websocketClient.ForwardData(serverConn) if err != nil { log.Printf("WebSocket
   ForwardData error: %v\n", err) } }()
...
- [x] 支持 socks5_username 和 socks5_password 覆盖 socks5_proxy 的 URL 中的用户名密码
...
1. **SOCKS5 客户端集成** - 使用提供的 socks5-websocket-proxy-golang/pkg/socks5 库
