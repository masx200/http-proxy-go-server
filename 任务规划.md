...
为项目添加 SOCKS5 协议作为上游代理的完整支持，包括配置文件、命令行参数和代理选择逻辑的集成。
...
4. **文档更新**: 更新README.md，添加所有代理类型的详细使用说明if proxyURL != nil && (strings.HasPrefix(proxyURL.String(), "ws://") || strings.HasPrefix(proxyURL.String(), "wss://")) {
    // 解析目标地址
...
- [ ] 扩展-upstream-type 参数支持 socks5 值
- [ ] 修改-upstream-address 参数处理逻辑支持 socks5:// 协议
- [ ] 更新 main 函数中的参数解析逻辑
- [ ] 添加 SOCKS5 代理配置的自动创建逻辑

#### 12. 集成 SOCKS5 客户端 🔄

- [ ] 添加 socks5-websocket-proxy-golang/pkg/socks5 依赖导入
- [ ] 实现 SOCKS5 代理客户端包装器
- [ ] 创建与标准库兼容的代理函数
- [ ] 实现 SOCKS5 连接管理
- [ ] 集成到现有的代理选择逻辑中
...
- [ ] 修改simple.Handle函数支持SOCKS5代理连接
- [ ] 修改auth.Handle函数支持SOCKS5代理连接
- [ ] 添加SOCKS5代理相关的导入包
- [ ] 实现HTTP数据包解析获取目标服务器域名和端口
- [ ] 调用socks5.NewSOCKS5Client(socks5Config)
- [ ] 将解析的参数传递给相应的代理函数
- [ ] 实现SOCKS5连接的数据转发逻辑

#### 15. 实现用户名密码覆盖功能支持 SOCKS5 🔄

- [ ] 扩展overrideProxyURLCredentials函数支持SOCKS5代理
- [ ] 支持socks5_username和socks5_password覆盖socks5_proxy的URL中的用户名密码
- [ ] 确保SOCKS5代理URL返回逻辑都使用用户名密码覆盖功能
- [ ] 添加SOCKS5代理URL的解析和验证逻辑
