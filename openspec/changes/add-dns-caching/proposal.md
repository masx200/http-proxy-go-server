...
The http-proxy-go-server currently performs DNS resolution for each request
without caching, resulting in redundant DNS queries for the same domains. Adding
DNS caching will improve performance, reduce external DNS service load, and
decrease response latency for repeated domain resolutions.
...
- **Affected code**:
  ...
  - All proxy modes (simple, auth, tls, tls+auth) will benefit from cached DNS
    resolution
