...
// resolveTargetAddressForHttpWithRoundRobin 从解析的IP数组中根据优先级策略选择一个地址（HTTP代理使用）
func resolveTargetAddressForHttpWithRoundRobin(addrs []string, target string, ipPriority options.IPPriority) string {
	if len(addrs) == 0 {
		return target
	}
	...
	log.Printf("Http RoundRobin (priority=%s) selected address %s from %v for target %s", ipPriority, selectedAddr, addrs, target)
